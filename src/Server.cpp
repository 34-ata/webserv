...
void Server::handleReadEvent(int fd)
{
	if (connectIfNotConnected(fd)
		|| m_connections.find(fd) == m_connections.end())
		return;
...
void Server::handleDirectory(int fd, const Location& loc, std::string uri,
							 std::string filePath)
{
	Response response;
			...
			if (access(indexFilePath.c_str(), R_OK) == 0)
			{
				m_response =
					response.status(OK)
						...
						.header("Connection",
								req->shouldClose() ? "close" : "keep-alive")
...
Server::ConnectionState::ConnectionState()
	: req(NULL), cache(""), response(""), responseOffset(0), timeStamp(0),
	  listenerFd(-1)
{
}
